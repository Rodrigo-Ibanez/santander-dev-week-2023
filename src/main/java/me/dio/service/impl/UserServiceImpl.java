...
public class UserServiceImpl implements UserService {
    ...
    @Transactional(readOnly = true)
    public User findByAccountNumber(String number) {
        return this.userRepository.findByAccountNumber(number)
                .orElseThrow(() -> new NotFoundException("User with account number " + number + " not found."));
    }

    @Transactional(readOnly = true)
    public User findByCardNumber(String number) {
        return this.userRepository.findByCardNumber(number)
                .orElseThrow(() -> new NotFoundException("User with card number " + number + " not found."));
    }

    @Transactional(readOnly = true)
    public List<User> findByFeatureName(String featureName) {
        return this.userRepository.findByFeatureName(featureName);
    }

    @Transactional(readOnly = true)
    public List<User> findByNewsTitle(String newsTitle) {
        return this.userRepository.findByNewsTitle(newsTitle);
    }
}